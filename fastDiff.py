...
def case3():
  ...
  im = np.array(Image.open(urllib2.urlopen(url))).astype(short int)
  im /= im.max()
  ...
  a=fig.add_subplot(232)
  f = dtdy
  b=plt.imshow((255*f/f.max()).astype(short int),interpolation='nearest',cmap='gray')
  a.set_title('After')

  a=fig.add_subplot(233)
  f = dtdy1
  plt.imshow((255*f/f.max()).astype(short int),interpolation='nearest',cmap='gray')
  a.set_title('After: axis 0')
  ...
  f = ((-(dtd.dctFilter)))
  f = (255*f/f.max()).astype(short int)
